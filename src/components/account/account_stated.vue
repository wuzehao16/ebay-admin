<template>
  <el-row class="warp">
	  ...
	  <el-col :span="24" class="toolbar" style="padding-bottom: 0px;">
	    <el-form :inline="true" :model="filterAccounts">
		  ...
		  <el-form-item label="交易状态">
			  <el-select v-model="filterAccounts.trade_status" placeholder="交易状态" clearable>
    ...
    <el-table ref="singleTable" :data="accounts" v-loading='loading' highlight-current-row @current-change="setHighlight" height='600' style="width: 100%">
    	...
    	<el-table-column property="user_name" label="用户姓名" width='140'></el-table-column>
    	<el-table-column property="tel" label="手机号码" width='140'></el-table-column>
    	<el-table-column property="account_in" label="收入（元）" width='140'></el-table-column>
    	<el-table-column property="account_out" label="支出（元）" width='140'></el-table-column>
    	<el-table-column property="account_left" label="账户余额（元）" width='140'></el-table-column>
    	<el-table-column label="交易状态" width='100'>
    		<template scope="scope">
    			<template v-if="scope.row.trade_status == 0">
    			处理中
    			</template>
    			<template v-if="scope.row.trade_status == 1">
    			成功
    			</template>
    			<template v-if="scope.row.trade_status == 2">
    			失败
    			</template>
    		</template>
    	</el-table-column>
...
<script>
  import { reqGetAccountSteted } from '../../api/api';
  ...
  export default{
    ...
    mounted() {
      if ( this.$route.params.account ) {
	      this.filterAccounts.user_name = this.$route.params.account.user_name
	      this.filterAccounts.tel = this.$route.params.account.tel      	
      }
      this.getAccounts();
    }
  }
